...
void queueadd(queue *, long long, long long);
queue *queuedel(queue *);
...
void queueadd(queue *pri, long long x, long long y)
{
	static queue *q = NULL;
	queue *q;
	if ((q = malloc(sizeof(queue))) == NULL) exit(1);	/*そこまで面倒見切れません*/
	if (pri != NULL) pri->p = q;
	q->x = x;
	q->y = y;
	q->p = NULL;
	...
	*/
	do {
		...
		else
		{
			...
			if (kill != 0) maps[now->x + XSIZE * now->y].r = 255;
			else
			{
				/* 色づけ */
			...
			/*発散しなかったのでキュー追加処理*/
			if (clrcmp(maps[(now->x + 1) + XSIZE * now->y], gray)) queueadd(0, 0);
			if (clrcmp(maps[(now->x - 1) + XSIZE * now->y], gray)) queueadd(0, 0);
			if (clrcmp(maps[now->x + XSIZE * (now->y + 1)], gray)) queueadd(0, 0);
			if (clrcmp(maps[now->x + XSIZE * (now->y - 1)], gray)) queueadd(0, 0);
		}
		now = queuedel();	/* キューを1減らす */
	} while(now != NULL);
