/* 数理情報学演習II H24_01_12.c */
/* マンデルブロ 標準偏差+色相回転+出力メニュー+画像マップ保持 */
/* 2012.01.12, 10RU004, 石橋祥太 */
...
void main(void)
{
	...
	unsigned char count;
	unsigned short XSIZE = 768, YSIZE = 768;
	...
	Mandelbrot(NULL, false, 0, 0, 0, 0, 0, 0, 0, XSIZE, YSIZE);
	/* 第二引数メモ：
		...
		8ビット目 (予約変数)
	*/
	Mandelbrot(file, true | mjmode << 1 | colmode << 2 | 1 << 4, 0.2,
				-0.52, 2.0, XMIN, XMAX, YMIN, YMAX, XSIZE, YSIZE);
	printf("出力中(2/2)... しばらくお待ちください\n", file);
	if (Mandelbrot(file, true | mjmode << 1 | colmode << 2 | bmpmode << 5, 0.2,
					-0.52, 2.0, XMIN, XMAX, YMIN, YMAX, XSIZE, YSIZE) != 0)
		 printf("エラーが発生しました\n");
...
{
	unsigned char flag, mjmode = mode >> 1 & 1, colmode = mode >> 2 & 3,
		kill = mode >> 4 & 1, bmp = mode >> 5 & 1, latch = mode >> 6 & 1;
